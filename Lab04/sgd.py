...
weights_out = np.random.randn(hidden_layer_size, output_size) * np.sqrt(
    1 / hidden_layer_size
)
...
def train(x_train, y_train, x_test, y_test, epochs, batch_size):
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch + 1}/{epochs}, Train Loss: {train_loss:.4f}, Test Loss: {test_loss:.4f}, Accuracy: {accuracy * 100:.4f}"
        )
